...
import { createStar, drawStar, refreshStar, resizeStar, resizeStarInGroup } from './star'
...
export const createShape = (
  ...
  switch (shape.type) {
    ...
    case 'star':
      return createStar(shape as any, roundCursorPosition, settings)
  }
}
...
const drawShapeByType = (ctx: CanvasRenderingContext2D, shape: ShapeEntity): void => {
  switch (shape.type) {
    ...
    case 'picture':
      ...
      break
    case 'star':
      drawStar(ctx, shape as ShapeEntity<'star'>)
      break
  }
}
...
export const resizeShape = <T extends ShapeEntity>(
  ...
  switch (originalShape.type) {
    ...
    case 'star':
      return resizeStar(cursorPosition, originalShape as ShapeEntity<'star'>, selectionMode as SelectionModeResize, settings, true, isAltPressed) as T
    default:
...
export const resizeShapes = (
  ...
  return groupShapes.map(shape => {
    ...
    switch (shape.type) {
      ...
      case 'star':
        return resizeStarInGroup(shape as ShapeEntity<'star'>, group, groupCtx)
      case 'brush':
...
export const refreshShape = (shape: DrawableShape & { id: string }, settings: UtilsSettings): ShapeEntity => {
  switch (shape.type) {
    ...
    case 'star':
      return refreshStar(shape as DrawableShape<'star'> & { id: string }, settings)
    case 'picture':
