...
For more details and links to Pull requests, see [Releases](https://github.com/aman-yadav/react-paint/releases)

## V1.0.0 - Aman Yadav Major Release
- Transfer complete repository ownership and metadata to **Aman Yadav** (`@aman-yadav/react-paint`)
- Update License to Copyright (c) 2026 Aman Yadav
- Add Star Shape tool (`'star'`) with full drawing, scaling, rotation, selection, and style customization
- Add optional Canvas Watermark attribution (`"Aman Paint - Developed by Aman Yadav"`)
- Expose programmatic Undo/Redo API (`undo()`, `redo()`, `canUndo`, `canRedo`, `clearHistory()`) in `useReactPaint`
- Add Keyboard Shortcuts help modal to the Editor UI
